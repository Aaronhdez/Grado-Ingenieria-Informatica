...
template <class T>
Array2D<T>::Array2D(char filename[300]){
  imageBMP I(filename);
  if(I.width()==0 || I.height()==0) return;
  *this=Array2D<T>(I.height(),I.width());
  for(int i=0;i<I.width();i++){
    for(int j=0;j<I.height();j++){
      int k=j*I.width()+i;
      data_[i*I.height()+j]= (T) ((int) ( (int) I[k].B_+I[k].R_+I[k].G_)/3);
    }
  }
//  for(int k=0;k<I.size();k++){
//    data_[k]= (T) ((int) ( (int) I[k].B_+I[k].R_+I[k].G_)/3);
//  }
}

template <class T>
void Array2D<T>::write_bmp(char name[300]){
  imageBMP I(dim2(),dim1());

  for(int i=0;i<I.width();i++){
    for(int j=0;j<I.height();j++){
      int k=j*I.width()+i;
      I[k].B_=I[k].R_=I[k].G_=(unsigned char) data_[i*I.height()+j];
      //data_[i*I.height()+j]= (T) ((int) ( (int) I[k].B_+I[k].R_+I[k].G_)/3);
    }
  }

//  for(int k=0;k<I.size();k++){
//    I[k].B_=I[k].R_=I[k].G_=(unsigned char) data_[k];
//  }
  I.write(name);
}
