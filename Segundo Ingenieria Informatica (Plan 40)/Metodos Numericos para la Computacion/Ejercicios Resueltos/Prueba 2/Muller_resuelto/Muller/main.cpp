...
int main()
{
    ...
    printf("Raiz  f(x)=x-tan(x)+1, x0=1, TOL=1e-12 NiterMax=10000 (debe funcionar)\n");
    x0=1.;
    NiterMax=10000;
    iter=mn_muller(f1,f1p,f1pp,x0,NiterMax,TOL);
    if(iter>=0){
      printf ("El resultado metodo de muller (alumno) : %1.16e\n", (double) x0);
      printf ("La funcion evaluada en el punto es : %1.16e\n", (double) f1(x0));
      printf ("El numero de iteraciones realizada es : %d\n",  iter);
    }
    else{
      printf ("El metodo de la muller falla (alumno)\n");
    }

    printf("\nRaiz  f(x)=x-tan(x)+1, x0=1, TOL=1e-12 NiterMax=3 (debe fallar)\n");
    x0=1.;
    NiterMax=3;
    iter=mn_muller(f1,f1p,f1pp,x0,NiterMax,TOL);
    if(iter>=0){
    ...
    printf("\nRaiz  f(x)=x-tan(x)+1, x0=0, TOL=1e-12 NiterMax=10000 (debe fallar)\n");
    x0=0.;
    NiterMax=10000;
    iter=mn_muller(f1,f1p,f1pp,x0,NiterMax,TOL);
    if(iter>=0){
    ...
    printf("\nRaiz  f(x)=x-sin(x)-1., x0=5, TOL=1e-12 NiterMax=10000 (debe funcionar)\n");
    x0=5;
    NiterMax=10000;
    iter=mn_muller(f2,f2p,f2pp,x0,NiterMax,TOL);
    if(iter>=0){
      printf ("El resultado metodo de muller (alumno): %1.16e\n", (double) x0);
      printf ("La funcion evaluada en el punto es : %1.16e\n", (double) f2(x0));
      printf ("El numero de iteraciones realizada es : %d\n",  iter);
