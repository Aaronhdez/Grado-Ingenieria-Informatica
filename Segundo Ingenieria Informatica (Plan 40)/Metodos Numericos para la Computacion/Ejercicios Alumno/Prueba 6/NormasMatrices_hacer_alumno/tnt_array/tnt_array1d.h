...
class Array1D
{
  ...
  public:
	...
	Array1D<T> operator-(const Array1D<T> &B) const;
	T          operator*(const Array1D<T> &B) const;
    Array1D<T> operator*(const T &B) const;
    Array1D<T> operator/(const T &B) const;
    Array1D<T> operator+(const T &B) const;
...
template <class T>
void Array1D<T>::print(char name[200])
{
	for(unsigned int i=0;i<n_;i++){
      printf("%s[%d]=%lf\n",name,i,(double) data_[i]);
    }
    printf("\n");
}
...
template<class T>
T Array1D<T>::operator*(const Array1D<T> &B) const
{
  T sum=0;
  for(int k=0;k<n_;k++)
    sum+=data_[k]*B[k];
...
template<class T>
Array1D<T> Array1D<T>::operator+(const Array1D<T> &B) const
{
	if (dim() != B.dim())
        return Array1D<T>();
...
template<class T>
Array1D<T> Array1D<T>::operator*(const T &B) const
{
    int N = dim();
...
template<class T>
inline   T Array1D<T>::norma() const
{
  T paso=0;
  int N = dim();
  for (int k=0; k<N; k++){ paso+= (*this)[k]*(*this)[k]; }
  if(paso>0.) return(sqrt(paso));
  return(0.);
}
