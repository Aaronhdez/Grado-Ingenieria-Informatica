...
real mn_cos(real x,int n){
    ...
    for(int i=1; i<n; i++){
        if(i%2==0){
            coseno += mn_potencia(x,2*i)/mn_factorial(2*i);
        } else {
            coseno -= mn_potencia(x,2*i)/mn_factorial(2*i);
        }
    }
...
real mn_sin(real x,int n){
    ...
    for(int i=1; i<n; i++){
        if(i%2==0){
            seno += mn_potencia(x,2*i+1)/mn_factorial(2*i+1);
        } else {
            seno -= mn_potencia(x,2*i+1)/mn_factorial(2*i+1);
        }
    }
...
real mn_limite1(real tolerancia){
    real actual = 2.;
    int c = 2;
    real previo = mn_potencia(1.+1./c,c);

    while(fabs(actual-previo)>tolerancia){
        actual = previo;
        c++;
        previo = mn_potencia(1.+1./c,c);
    }
    return previo;
}
