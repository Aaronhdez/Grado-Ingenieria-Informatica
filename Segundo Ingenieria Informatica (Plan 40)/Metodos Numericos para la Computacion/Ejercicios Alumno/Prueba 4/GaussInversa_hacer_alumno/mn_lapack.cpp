...
const Array2D< real > &A  /** MATRIZ  */)
{
  ...
  ///INICIAMOS EL VECTOR PIVOTE
  for(int k=0;k<piv.dim(); k++){
    piv[k] = k;
  }
  ///Convertimos el sistema en uno triangular por Gauss
    for(int k=0;k<Acopy.dim1();k++){
        real max = fabs(Acopy[piv[k]][k]);
        int kmax = k;
        for(int j=k+1;j<Acopy.dim1();j++){
            if(fabs(Acopy[piv[j]][k])>max){
                kmax = j;
                max = fabs(Acopy[piv[j]][k]);
            }
        }
        ///Verificamnos si a max ha cambiado y modificamos el vector pivotacion
        if(kmax != k){
            int temp = piv[kmax];
            piv[kmax] = piv[k];
            piv[k] = temp;
        }
        ///convertimos en 0 la columna de la diagonal
        for(int j=k+1; j<Acopy.dim1(); j++){
            real mul = -Acopy[piv[j]][k]/Acopy[piv[k]][k];
            for(int n = k+1;n<Acopy.dim1();n++){
                Acopy[piv[j]][n] += mul*Acopy[piv[k]][n];
            }
            for(int n = 0;n<Acopy.dim1();n++){
                B[piv[j]][n] += mul*B[piv[k]][n];
            }
        }
    }

    ///remontamos
    for(int k=Acopy.dim1()-1; k>=0; k--){
        if(Acopy[piv[k]][k] == 0){
            return Array2D<real>();
        }
        for(int n=0;n<Acopy.dim1();n++){
            A_1[k][n] = B[piv[k]][n];
            for(int i=k+1;i<Acopy.dim1();i++){
                A_1[k][n] -= Acopy[piv[k]][i]*A_1[i][n];
            }
