...
{
  /** HACER ALUMNO */
    ///COMPROBAMOPS QUE LA MATRIZ SEA CUADRADA
    if(A.dim1() != A.dim2() || A.dim1() != b.dim() || b.dim() == 0){
        return Array1D<real>();
    }
    ...
    ///Iniciamos la iteración de Gauss
    for(int k=0; k<b.dim(); k++){
        ///Comprobamos el máximo de la fila
        real max = fabs(A[k][k]);
        int kmax = k;
        ...
        ///Vemos si ha cambiado el max y hacemos el pivote. Hay que ver si el max es o no 0
        if(A[kmax][k] == 0.){
            return Array1D< real >();
        }
        if(kmax!=k){
            ...
            ///Hacemos el cambio en el vector soluciones
            real paso=b[kmax];
            b[kmax] = b[k];
            b[k] = paso;
        }
        ///Hacemos 0 la columna de la diagonal principal y aplicamos la solución al vectro también
    ...
    ///remontamos la resultante
    if(A[b.dim()-1][b.dim()-1]==0.){
        /// Si el valor en l amatriz es 0 pero en el vector resultado no lo es, es un SCI
        /// En caso contrario, es un 1.
        if(b[b.dim()-1]!=0){
            return Array1D<real>();
        } else{
            u[b.dim()-1] =1.;
        ...
        ///Si ambos son distintos de cero, pues despejamos.
